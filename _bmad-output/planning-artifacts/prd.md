...
### 5.1 AI Metadata Generation (Phase 0)

| FR | Requirement |
|---|---|
| **FR-IP0** | The Librarian provides only the raw `.md` content file for curriculum lessons, or the raw `.pdf` for library. |
| **FR-IP0.1** | The AI Assistant (acting as Librarian Agent) is responsible for reading the source document and automatically generating the `.json` metadata sidecar (extracting ACS codes, generating titles, and identifying necessary `reg_keys` and `doc_keys`). |
