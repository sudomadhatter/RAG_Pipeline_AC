...
ArtifactMetadata:
  title: <title>
  type: implementation_plan | walkthrough | bug_list | code_review
  date: <YYYY-MM-DD>
---
```
...
- NEVER run `git commit` / `git push` yourself — provide the command for Daniel.
- NEVER deliver code-review findings inline-only — always persist them as a `code-review.md` artifact.

## Why this matters
Artifact files are Daniel's primary interface for reviewing session work. Skipping this
breaks the entire collaboration model.
